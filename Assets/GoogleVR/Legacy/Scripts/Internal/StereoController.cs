  ...
  public void UpdateStereoValues() {
    ...
    for (int i = 0, n = eyes.Length; i < n; i++) {
      eyes[i].UpdateStereoValues();
			eyes [i].GetComponent<Camera> ().cullingMask = 0;
    }
  }
...
/*This Section of code has been modify in order to perform different layer ouputs to different GVR Eye Edited by 13068831d*/
	...
	//Change layer culling masks for left and right eye
	go.GetComponent<Camera> ().cullingMask &= 
	~(1 << LayerMask.NameToLayer((eye == GvrViewer.Eye.Left ? "Right":"Left")+"Eye"));
